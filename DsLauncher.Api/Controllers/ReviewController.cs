...
public class ReviewController(Repository<Review> repository) : EntityController<Review>(repository)
{
    ...
    public async Task<Review?> GetByProductAndUser(Guid productId, Guid userId, int skip = 0, int take = 1000, CancellationToken ct = default) =>
        (await repo.GetAll(restrict: x => x.ProductId == productId.Deobfuscate().Id && x.UserGuid == userId, ct: ct)).Skip(skip).Take(take).FirstOrDefault();
